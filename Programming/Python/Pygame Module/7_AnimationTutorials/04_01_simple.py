...
def get_sequence(frames_names, sequence, optimize=True):
    ...
    for name in frames_names:
        if not cache.has_key(name):  # check if it has been loaded already
...
def get_names_list(basename, ext, num, num_digits=1, offset=0):
    ...
    format_string = "%s%0" + str(num_digits) + "d.%s"
    for i in range(offset, num + 1):
        names.append(format_string % (basename, i, ext))
    return names
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            # only do something if the event is if of type QUIT
